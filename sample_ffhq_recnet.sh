...
"
pathlog="output/ffqh256-1k/mixinit-recnet"
for file in $files
do
	pathout="output/ffqh256-1k/mixinit-recnet/$file"
	python scripts/sample_diffusion.py \
		...
		-c 200 \
		-e 1.0
	python -m pytorch_fid $pathgt $pathout/img --device cuda:0 | grep FID | sed "s/^/file $file /" >> $pathlog.txt
	python unet_cls_test.py $pathout/img $pathgt | grep TotalAcc | sed "s/^/file $file /" >> $pathlog.acc.txt
done
